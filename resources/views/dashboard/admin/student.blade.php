        ...
        @if (count($students))
            ...
            @foreach ($students as $student)
                <div class="body {{ $number++ % 2 == 0 ? 'body-bg-two' : 'body-bg-one' }}">
                    <span>{{ $student->id }}</span>
                    <span>{{ $student->name }}</span>
                    <span style="">{{ count($student->courses) }}</span>
                    <span class="status" style="position: relative;">
                        {{-- <div class="number">1</div> --}}
                        <a href="{{route('dashboard.student.show',$student->id)}}" style="color:#009688"><i class="fas fa-eye" title="View Profile" style="font-size: 30px;"></i></a>
                    </span>
                </div>
            @endforeach
        @else
