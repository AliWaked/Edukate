<x-dashboard.layout title='Courses' :items="['My Courses']">
    ...
    <section class="show pb-4">
        <a href="{{ route('dashboard.instructor.course.create') }}">add courses</a>
        @if (count($courses))
            <div class="search">
            ...
            @foreach ($courses as $course)
                <div class="body @if (++$num % 2 == 0) body-bg-two @else body-bg-one @endif">
                    ...
                    <span style="text-align: center;">
                        <div class="status {{ $course->status == 'not acceptable' ? 'draft' : 'active' }}">
                            {{ $course->status }}</div>
                    </span>
                    <span class="span-form">
                        <form action="{{ route('dashboard.instructor.course.show', $course->slug) }}" method="get">
                            @csrf
                            <button type="submit" class="show"><i class="fas fa-eye"></i></button>
                        </form>
                        <form action="{{route('dashboard.instructor.course.edit',$course->id)}}" method="get">
                            {{-- @csrf
                            @method('put') --}}
                            <button type="submit" class="edit"><i class="fas fa-pen"></i></button>
                        </form>
                        <form action="{{ route('dashboard.instructor.course.destroy', $course->id) }}" method="post">
                            @csrf
                            @method('delete')
            ...
            @endforeach
        @else
            <div class="message ">Not Have Any Course</div>
